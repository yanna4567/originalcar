...
class LineFollower(Node):
    def __init__(self):
        ...
        # 创建发布者 - CV系统只负责巡线，发布到CV专用话题
        self.pub_cmd_vel = self.create_publisher(Twist, '/cv_cmd_vel', 10)  # CV专用话题
        ...
        self.get_logger().info("CV巡线系统初始化完成")
    ...
    def stop_robot(self):
        ...
        self.get_logger().info("CV巡线系统已停止")
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # 创建并发布速度控制消息 - CV系统只负责巡线
            twist = Twist()
            twist.linear.x = self.detector.linear_speed  # 设置线速度
            twist.angular.z = control_info['angular_z']  # 设置角速度

            self.pub_cmd_vel.publish(twist)  # 发布到CV专用话题
            self.get_logger().debug(f"CV巡线控制: linear={twist.linear.x:.2f}, angular={twist.angular.z:.3f}")

        except Exception as e:
            self.get_logger().warn(f"CV图像处理失败: {e}")
